...
struct mmap_assets_table {
    char asset_name[32];          /* !< Tên tài sản */
    uint32_t asset_size;          /* !< Kích thước của tài sản */
    uint32_t asset_offset;        /* !< Bù đắp tài sản */
    uint16_t asset_width;         /* !< Chiều rộng của nội dung */
    uint16_t asset_height;        /* !< Chiều cao của nội dung */
};
...
bool Assets::LoadSrmodelsFromIndex(Assets* assets, cJSON* root) {
    ...
    // Nếu root không được cung cấp, hãy phân tích index.json
    if (root == nullptr) {
...
void Assets::LvglStrategy::UnApplyPartition(Assets* assets) {
    ...
    assets_.clear();
    (void)assets; // Tham số không sử dụng
}
...
bool Assets::EmoteStrategy::InitializePartition(Assets* assets) {
    ...
    if (emote_display && emote_display->GetEmoteHandle() != nullptr) {
        const emote_data_t data = {
            ...
            .flags = {
                .mmap_enable = true, // chắc là đúng đây!!!
            },
        };
...
bool Assets::Download(std::string url, std::function<void(int progress, size_t speed)> progress_callback) {
    ...
    // Hủy ánh xạ bộ nhớ của phân vùng tài nguyên hiện tại
    UnApplyPartition();

    // Tải xuống các tệp tài nguyên mới
    ...
    // Xác định kích thước cung là 4KB (kích thước cung tiêu chuẩn cho ESP32)
    const size_t SECTOR_SIZE = esp_partition_get_main_flash_sector_size();
    
    // Tính toán số lượng các lĩnh vực cần được xóa
    ...
    // Ghi các tệp tài nguyên mới vào phân vùng và ghi chúng trong khi xóa chúng.
    char buffer[512];
    ...
    while (true) {
        ...
        if (esp_timer_get_time() - last_calc_time >= 1000000 || total_written == content_length || ret == 0) {
            ...
            size_t speed = recent_written; // byte mỗi giây
            ...
            recent_written = 0; // Đặt lại số byte được ghi gần đây
        }
    }
