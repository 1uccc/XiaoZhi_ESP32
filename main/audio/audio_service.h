...
/* * Có hai loại luồng dữ liệu âm thanh:
 * 1. (MIC) -> [Bộ xử lý] -> {Hàng đợi mã hóa} -> [Bộ mã hóa Opus] -> {Gửi hàng đợi} -> (Máy chủ)
 * 2. (Máy chủ) -> {Hàng đợi giải mã} -> [Bộ giải mã Opus] -> {Hàng đợi phát lại} -> (Loa)
 *
 * Chúng tôi sử dụng một tác vụ cho MIC / Loa / Bộ xử lý và một tác vụ cho Bộ mã hóa Opus / Bộ giải mã Opus.
 * 
 * Hàng đợi giải mã và Hàng đợi gửi là các hàng đợi chính, vì các gói Opus khá nhỏ hơn các gói PCM.
 * */
