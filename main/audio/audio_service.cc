...
bool AudioService::ReadAudioData(std::vector<int16_t>& data, int sample_rate, int samples) {
    ...
    /* Cập nhật thời gian nhập liệu cuối cùng */
...
void AudioService::AudioInputTask() {
    while (true) {
        ...
        /* Được sử dụng để kiểm tra âm thanh ở chế độ NetworkConfiguring bằng cách nhấp vào nút BOOT */
        if (bits & AS_EVENT_AUDIO_TESTING_RUNNING) {
            ...
            if (ReadAudioData(data, 16000, samples)) {
                // Nếu kênh đầu vào là 2, chúng ta cần lấy dữ liệu kênh bên trái
...
void AudioService::AudioOutputTask() {
    while (true) {
        ...
        /* Cập nhật thời gian xuất cuối cùng */
        last_output_time_ = std::chrono::steady_clock::now();
        debug_statistics_.playback_count++;

#if CONFIG_USE_SERVER_AEC
        /* Ghi lại dấu thời gian cho máy chủ AEC */
        if (task->timestamp > 0) {
            lock.lock();
...
void AudioService::OpusCodecTask() {
    while (true) {
        ...
        /* Giải mã âm thanh từ hàng đợi giải mã */
        ...
        /* Mã hóa âm thanh để gửi hàng đợi */
...
void AudioService::PushTaskToEncodeQueue(AudioTaskType type, std::vector<int16_t>&& pcm) {
    ...
    /* Đẩy tác vụ vào hàng đợi mã hóa */
    std::unique_lock<std::mutex> lock(audio_queue_mutex_);

    /* Nếu nhiệm vụ là gửi hàng đợi, chúng ta cần đặt dấu thời gian */
...
void AudioService::EnableWakeWordDetection(bool enable) {
    ...
    if (enable) {
        ...
        // Đặt lại bộ lấy mẫu lại đầu vào để xóa dữ liệu được lưu trong bộ nhớ đệm khỏi chế độ trước đó (ví dụ: AudioProcessor)
        // Điều này ngăn ngừa tràn bộ đệm khi chuyển đổi giữa các kích thước nguồn cấp dữ liệu khác nhau
        {
            std::lock_guard<std::mutex> lock(input_resampler_mutex_);
            if (input_resampler_ != nullptr) {
...
void AudioService::EnableVoiceProcessing(bool enable) {
    ...
    if (enable) {
        ...
        /* Chúng ta nên đảm bảo không có âm thanh nào đang phát */
        ResetDecoder();
        audio_input_need_warmup_ = true;
        // Đặt lại bộ lấy mẫu lại đầu vào để xóa dữ liệu được lưu trong bộ nhớ đệm khỏi chế độ trước đó (ví dụ: WakeWord)
        // Điều này ngăn ngừa tràn bộ đệm khi chuyển đổi giữa các kích thước nguồn cấp dữ liệu khác nhau
        {
            std::lock_guard<std::mutex> lock(input_resampler_mutex_);
            if (input_resampler_ != nullptr) {
...
void AudioService::EnableAudioTesting(bool enable) {
    ...
    if (enable) {
        ...
        /* Sao chép audio_testing_queue_ sang audio_decode_queue_ */
        std::lock_guard<std::mutex> lock(audio_queue_mutex_);
...
void AudioService::PlaySound(const std::string_view& ogg) {
    ...
    while (true) {
        ...
        // Phân tích các gói bằng cách sử dụng viền
        size_t cur = body_off;
        size_t seg_idx = 0;
        while (seg_idx < page_segments) {
            ...
            if (!seen_head) {
                ...
                if (pkt_len >= 19 && std::memcmp(pkt_ptr, "OpusHead", 8) == 0) {
                    seen_head = true;
                    // OpusHead结构:[0-7] "OpusHead", [8] phiên bản, [9] kênh_count, [10-11] pre_skip
                    // [12-15] tốc độ mẫu đầu vào, [16-17] đầu ra_gain, [18] ánh xạ_gia đình
                    if (pkt_len >= 12) {
                        uint8_t version = pkt_ptr[8];
                        uint8_t channel_count = pkt_ptr[9];
                        if (pkt_len >= 16) {
                            // Đọc tốc độ mẫu đầu vào (little-endian)
