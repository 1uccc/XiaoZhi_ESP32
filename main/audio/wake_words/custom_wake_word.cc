...
bool CustomWakeWord::Initialize(AudioCodec* codec, srmodel_list_t* models_list) {
    ...
    if (mn_name_ == nullptr) {
        ...
        ESP_LOGI(TAG, "Please refer to https:// pcn7cs20v8cr.feishu.cn/wiki/CpQjwQsCJiQSWSkYEvrcxcbVnwh để thêm từ đánh thức tùy chỉnh");
        return false;
    }
...
void CustomWakeWord::StoreWakeWordData(const std::vector<int16_t>& data) {
    // lưu trữ dữ liệu âm thanh vào Wake_word_pcm_
    wake_word_pcm_.push_back(data);
    // giữ khoảng 2 giây dữ liệu, thời lượng phát hiện là 30ms (sample_rate == 16000, chunksize == 512)
