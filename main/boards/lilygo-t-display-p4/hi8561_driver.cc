...
static const hi8561_lcd_init_cmd_t vendor_specific_init_default[] = {
    ...
    // {0xCC, (uint8_t[]){0x33}, 1, 0},//4làn
    {0xCC, (uint8_t[]){0x31}, 1, 0}, // 2lane
    // {0xCC, (uint8_t[]){0x30}, 1, 0}, // 1 làn
...
{
    // ESP_LOGI(TAG, "phiên bản: %d.%d.%d", ESP_LCD_hi8561_VER_MAJOR, ESP_LCD_hi8561_VER_MINOR,
...
static esp_err_t panel_hi8561_send_init_cmds(hi8561_panel_t *hi8561)
{
    ...
    // uint8_t làn_command = HI8561_DSI_2_LANE;
    // bool is_cmd_overwriting = false;

    // chuyển đổi (hi8561->lane_num)
    // {
    // trường hợp 0:
    // trường hợp 2:
    // làn_lệnh = HI8561_DSI_2_LANE;
    // phá vỡ;
    // trường hợp 4:
    // làn_lệnh = HI8561_DSI_4_LANE;
    // phá vỡ;
    // mặc định:
    // ESP_LOGE(TAG, "Số làn không hợp lệ %d", hi8561->lane_num);
    ...
    // init_cmds_size = sizeof(vendor_special_init_default) / sizeof(hi8561_lcd_init_cmd_t);
    // }
