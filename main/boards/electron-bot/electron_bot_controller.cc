/* Bộ điều khiển Robot Electron - Phiên bản giao thức MCP */
...
private:
    ...
    enum ActionType {
        // Động tác tay 1-12
        ACTION_HAND_LEFT_UP = 1,      // Hãy giơ tay trái của bạn lên
        ACTION_HAND_RIGHT_UP = 2,     // Hãy giơ tay phải của bạn lên
        ACTION_HAND_BOTH_UP = 3,      // giơ tay
        ACTION_HAND_LEFT_DOWN = 4,    // Đặt tay trái của bạn
        ACTION_HAND_RIGHT_DOWN = 5,   // Đặt tay phải của bạn
        ACTION_HAND_BOTH_DOWN = 6,    // Bỏ tay xuống
        ACTION_HAND_LEFT_WAVE = 7,    // vẫy tay trái
        ACTION_HAND_RIGHT_WAVE = 8,   // vẫy tay phải
        ACTION_HAND_BOTH_WAVE = 9,    // vẫy tay
        ACTION_HAND_LEFT_FLAP = 10,   // tát tay trái
        ACTION_HAND_RIGHT_FLAP = 11,  // tát vào tay phải
        ACTION_HAND_BOTH_FLAP = 12,   // vỗ tay

        // Chuyển động cơ thể 13-14
        ACTION_BODY_TURN_LEFT = 13,    // Rẽ trái
        ACTION_BODY_TURN_RIGHT = 14,   // Rẽ phải
        ACTION_BODY_TURN_CENTER = 15,  // Trở về trung tâm

        // Chuyển động đầu 16-20
        ACTION_HEAD_UP = 16,          // tra cứu
        ACTION_HEAD_DOWN = 17,        // cúi đầu
        ACTION_HEAD_NOD_ONCE = 18,    // gật đầu một lần
        ACTION_HEAD_CENTER = 19,      // Trở về trung tâm
        ...
        while (true) {
            if (xQueueReceive(controller->action_queue_, &params, pdMS_TO_TICKS(1000)) == pdTRUE) {
                ...
                if (params.action_type >= ACTION_HAND_LEFT_UP &&
                    ...
                    // thiết lập lại hành động
                    controller->electron_bot_.Home(true);
                }
                controller->is_action_in_progress_ = false;  // Hành động đã hoàn tất
            }
...
public:
    ...
    void RegisterMcpTools() {
        ...
        // Công cụ thống nhất chuyển động tay
        mcp_server.AddTool(
            ...
            [this](const PropertyList& properties) -> ReturnValue {
                ...
                // Tính toán các chuyển động cụ thể dựa trên loại chuyển động và loại tay
                int base_action;
                switch (action_type) {
                    case 1:
                        base_action = ACTION_HAND_LEFT_UP;
                        break;  // Hãy giơ tay lên
                    case 2:
        ...
        mcp_server.AddTool("self.electron.stop", "立即停止", PropertyList(),
                           [this](const PropertyList& properties) -> ReturnValue {
                               // Xóa hàng đợi nhưng vẫn giữ nguyên nhiệm vụ
