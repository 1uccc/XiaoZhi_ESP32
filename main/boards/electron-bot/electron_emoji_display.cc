...
void ElectronEmojiDisplay::InitializeElectronEmojis() {
    ...
    // Biểu hiện trung lập/bình tĩnh -> trạng thái tĩnh
    ...
    // Biểu tượng cảm xúc tích cực/vui vẻ -> hạnh phúc
    ...
    // Suy nghĩ/biểu hiện bối rối -> buxue
    ...
    // Thêm bộ sưu tập biểu tượng cảm xúc vào chủ đề
    ...
    // Đặt biểu thức mặc định thành trạng thái tĩnh
...
void ElectronEmojiDisplay::SetupChatLabel() {
    ...
    lv_obj_set_width(chat_message_label_, width_ * 0.9);                        // Giới hạn chiều rộng ở mức 90% chiều rộng màn hình
    lv_label_set_long_mode(chat_message_label_, LV_LABEL_LONG_SCROLL_CIRCULAR);            // Đặt ở chế độ ngắt dòng
    lv_obj_set_style_text_align(chat_message_label_, LV_TEXT_ALIGN_CENTER, 0);  // Đặt căn chỉnh văn bản vào giữa
...
void ElectronEmojiDisplay::SetStatus(const char* status) {
    ...
    if (strcmp(status, Lang::Strings::LISTENING) == 0) {
        ...
        lv_label_set_text(status_label_, "\xEF\x84\xB0");  // Biểu tượng micrô U+F130
        ...
        lv_obj_set_style_text_font(status_label_, &OTTO_ICON_FONT, 0);
        lv_label_set_text(status_label_, "\xEF\x80\xA8");  // Biểu tượng nói U+F028
        lv_obj_clear_flag(status_label_, LV_OBJ_FLAG_HIDDEN);
        lv_obj_add_flag(network_label_, LV_OBJ_FLAG_HIDDEN);
        lv_obj_add_flag(battery_label_, LV_OBJ_FLAG_HIDDEN);
        return;
        ...
        lv_obj_set_style_text_font(status_label_, &OTTO_ICON_FONT, 0);
        lv_label_set_text(status_label_, "\xEF\x83\x81");  // Biểu tượng kết nối U+F0c1
        lv_obj_clear_flag(status_label_, LV_OBJ_FLAG_HIDDEN);
        return;
