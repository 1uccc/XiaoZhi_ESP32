...
SscmaCamera::SscmaCamera(esp_io_expander_handle_t io_exp_handle) {
    ...
    spi_io_config.wait_delay = 10; // Độ trễ giữa hai lần chuyển ít nhất là 4ms, nhưng hiện tại FREERTOS_HZ=100, độ chính xác về độ trễ chỉ có thể đạt tới 10ms.
    ...
    callback.on_event = [](sscma_client_handle_t client, const sscma_client_reply_t *reply, void *user_ctx) {
        ...
        switch ((width+height)) {
            case (416+416): 
            {
                ...
                // Thường xuyên cập nhật thông số cấu hình phát hiện để tránh truy cập NVS thường xuyên
                int64_t cur_tm = esp_timer_get_time();

                // Cố gắng lấy dữ liệu khung phát hiện (mô hình phát hiện mục tiêu)
                if (sscma_utils_fetch_boxes_from_reply(reply, &boxes, &box_count) == ESP_OK && box_count > 0) {
                    ...
                    // Cố gắng lấy dữ liệu phân loại (mô hình phân loại)
                    ...
                    free(classes);
                     ...
                     // Cố gắng lấy dữ liệu điểm chính (mô hình ước tính tư thế)
                ...
                // Nếu bạn cần bắt đầu thời gian hồi phục, hãy bắt đầu tính thời gian ngay bây giờ
                if (self->need_start_cooldown) { // Cuộc gọi lại bị tạm dừng, cờ được duy trì và thời gian bắt đầu sau khi chờ cuộc gọi lại tiếp tục.
                    self->state_start_time = cur_tm;
                    self->need_start_cooldown = false;
                    ESP_LOGI(TAG, "Starting cooldown timer");
                }
                
                // Logic phát hiện điều khiển máy trạng thái - chỉ được kích hoạt khi có mặt một người
                switch (self->detection_state) {
                    case SscmaCamera::IDLE:
                        if (is_object_detected) {
                            // Mọi người xuất hiện và bắt đầu xác minh (đây là một sự chuyển đổi từ không có gì thành có gì đó)
                            self->detection_state = SscmaCamera::VALIDATING;
                            self->state_start_time = cur_tm; // Ghi lại thời điểm vật thể xuất hiện
                            self->last_detected_time = cur_tm; // Khởi tạo thời gian phát hiện lần cuối
                    ...
                    case SscmaCamera::VALIDATING:
                        if (is_object_detected) {
                            // Cập nhật thời gian phát hiện lần cuối
                            ...
                            // Nhân sự rời đi trong quá trình xác minh, kiểm tra thời gian gỡ lỗi
                            if (self->last_detected_time > 0 && 
                                (cur_tm - self->last_detected_time) >= self->detect_debounce_sec * 1000000LL) {
                                // Thời gian gỡ lỗi đã trôi qua, xác nhận rằng người đó đã rời đi và trở về trạng thái rảnh rỗi
                    ...
                    case SscmaCamera::COOLDOWN:
                        // Trong thời gian làm mát, cần phải đáp ứng hai điều kiện: 1) vật thể rời đi và 2) 15 giây đã trôi qua.
                        if (!is_object_detected && 
                            (cur_tm - self->state_start_time) >= (self->detect_invoke_interval_sec * 1000000LL)) {
                            // Đối tượng rời đi và hết thời gian làm mát, trở về trạng thái không hoạt động.
                            self->detection_state = SscmaCamera::IDLE;
                            ESP_LOGI(TAG, "Cooldown complete and object left, back to idle - ready for next appearance");
                        }
                        // Trong các trường hợp khác, tiếp tục hạ nhiệt
                        break;
                }
                ...
                if( is_need_wake ) {
                    ...
                    // Vào trạng thái làm mát, đánh dấu sự cần thiết phải bắt đầu thời gian làm mát; các biến sau sẽ được sử dụng sau khi phiên kết thúc và thời gian sẽ bắt đầu sau khi chờ cuộc gọi lại tiếp tục.
            ...
            case (640+480):
                ...
                if (sscma_utils_fetch_image_from_reply(reply, &img, &img_size) == ESP_OK)
                {
                    ...
                    // Xóa hàng đợi để đảm bảo chỉ lưu dữ liệu mới nhất
                    SscmaData dummy;
                    ...
                    // Lưu ý: Việc phát hành img là trách nhiệm của người nhận
                }
                break;
            default:
    ...
    // Khởi tạo bộ nhớ cho dữ liệu JPEG
    jpeg_data_.len = 0;
    ...
    // Khởi tạo bộ nhớ cho hình ảnh xem trước
        ...
        while (true)
        {
            ...
            if (this_->inference_en && Application::GetInstance().GetDeviceState() == kDeviceStateIdle ) {
                if (!is_inference) {
                    ...
                    sscma_client_set_sensor(this_->sscma_client_handle_, 1, 1, true); // Đặt độ phân giải 416X416
    ...
    mcp_server.AddTool("self.model.param_set",
        ...
        [this](const PropertyList& properties) -> ReturnValue {
            ...
            try {
                ...
                // tham số ngưỡng không được cung cấp, bỏ qua
            }
            ...
            try {
                ...
                // tham số khoảng không được cung cấp, bỏ qua
            }
            ...
            try {
                ...
                // tham số thời lượng không được cung cấp, bỏ qua
            }
            ...
            try {
                ...
                // tham số target_type không được cung cấp, bỏ qua
            }

            return "{\"status\": \"success\", \"message\": \"Detection configuration updated\"}";
        });

    // Thu thập chuyển đổi suy luận
    mcp_server.AddTool("self.model.enable",
        ...
        [this](const PropertyList& properties) -> ReturnValue {
            ...
            try {
                ...
                // bật không được cung cấp -> coi như truy vấn
            }
            // Quay lại cấu hình hiện tại
...
bool SscmaCamera::Capture() {
    ...
    // himax có thể có dữ liệu được lưu trong bộ nhớ cache, chỉ cần lấy những bức ảnh mới nhất.
    if (sscma_client_sample(sscma_client_handle_, 1) ) {
        ESP_LOGE(TAG, "Failed to capture image from SSCMA client");
        return false;
    }
    vTaskDelay(pdMS_TO_TICKS(500)); // Đang chờ máy khách SSCMA xử lý dữ liệu
 ...
 * Ví dụ về định dạng: {"success": true, "result": "Analysis results"}
