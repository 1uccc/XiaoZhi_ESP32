...
class CustomLcdDisplay : public SpiLcdDisplay {
    public:
        CustomLcdDisplay(esp_lcd_panel_io_handle_t io_handle, 
            ...
            // Đặt lại nhãn tắt tiếng và pin thành top_bar_ để cho phép định vị tuyệt đối
            ...
            // Điều chỉnh vị trí hộp thoại dưới cùng cho màn hình tròn tránh bị che khuất bởi các góc bo tròn
            lv_obj_set_style_pad_bottom(bottom_bar_, 30, 0);
            lv_obj_set_width(chat_message_label_, LV_HOR_RES * 0.75); // Giới hạn chiều rộng để tránh văn bản bị cong
        }
};
...
private:
    ...
    void InitializeI2c() {
        // Khởi tạo thiết bị ngoại vi I2C
    ...
    void OnKnobRotate(bool clockwise) {
        ...
        // Đảm bảo âm lượng nằm trong phạm vi hợp lệ
        if (new_volume > 100) {
            new_volume = 100;
        ...
        // Kiểm tra các thay đổi thực tế trước khi hiển thị thông báo
    ...
    void InitializeButton() {
        // Đặt con trỏ phiên bản tĩnh
        instance_ = this;
        
        // Trình xem được bật bằng cách nhấn và giữ bánh xe cuộn. Bạn cần đợi bánh xe cuộn được nhả ra, nếu không người dùng có thể vô tình bấm vào khi nhả bánh xe.
        ...
        iot_button_register_cb(btns, BUTTON_LONG_PRESS_HOLD, nullptr, [](void* button_handle, void* usr_data) {
            auto self = static_cast<SensecapWatcher*>(usr_data);
            self->long_press_cnt_++; // Thêm một cái cứ sau 20 mili giây
            // Nhấn và giữ trong 10 giây để khôi phục cài đặt gốc: 2+0,02*400 = 10
    ...
    void Initializespd2010Display() {
        ...
        const esp_lcd_panel_dev_config_t panel_config = {
            .reset_gpio_num = BSP_LCD_GPIO_RST, // Được chia sẻ với Touch reset
        ...
        // Đặt chỉ mục cột bắt đầu của mỗi lần làm mới là bội số của 4 và tổng số cột là bội số của 4 để đáp ứng yêu cầu của SPD2010
        lv_display_add_event_cb(lv_display_get_default(), [](lv_event_t *e) {
            lv_area_t *area = (lv_area_t *)lv_event_get_param(e);
            uint16_t x1 = area->x1;
            uint16_t x2 = area->x2;
            // làm tròn phần bắt đầu của vùng xuống số 4N gần nhất
            area->x1 = (x1 >> 2) << 2;
            // làm tròn phần cuối của khu vực lên đến số 4M+3 gần nhất
            area->x2 = ((x2 >> 2) << 2) + 3;
    ...
    uint16_t BatterygetVoltage(void) {
        ...
        if (initialized) {
            ...
            // ESP_LOGI(TAG, "điện áp: %dmV", điện áp);
    ...
    void InitializeCamera() {
        ...
        // !!!LƯU Ý: Thẻ SD sử dụng cùng bus SPI như máy khách sscma, vì vậy trước tiên chúng tôi cần tắt pin CS của thẻ SD
        const gpio_config_t io_config = {
...
public:
    SensecapWatcher() {
        ...
        InitializeSpi();
        InitializeExpander();
        InitializeCmd();  // Sử dụng thử nghiệm sản xuất tại nhà máy
        InitializeButton();
        InitializeKnob();
        Initializespd2010Display();
        GetBacklight()->RestoreBrightness();  // Đối với phiên bản không có camera, LaunchizeCamera mất 3s nên khôi phục độ sáng đèn nền trước
        InitializeCamera();
    }
    ...
    // Model LED RGB là ws2813 mini, kết nối GPIO 40, điện áp nguồn 3.3v, không kết nối đường tín hiệu kép BIN
    // Có thể tương thích trực tiếp với ws2812 được sử dụng bởi SingleLED
...
// Xác định các biến thành viên tĩnh
