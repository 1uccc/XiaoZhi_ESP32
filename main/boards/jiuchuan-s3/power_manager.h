...
public:
    ...
    bool IsCharging() {
        ...
        if (battery_level_ == 100) {
            // ESP_LOGI(TAG, "Pin đã đầy, không hiển thị đang sạc");
            return false;
        }
        return is_charging_;
    }
    ...
    void RegisterAllCallbacks() {
        // Đăng ký chức năng gọi lại thay đổi trạng thái nguồn (phiên bản tối ưu hóa)
        power_controller_->OnStateChange([this](PowerState newState) {
            switch(newState) {
                ...
                // Hủy kích hoạt PWR_EN
                    /* Ngăn chặn việc vô tình đánh thức sau khi tắt máy */
                    ESP_ERROR_CHECK(esp_sleep_enable_ext0_wakeup(PWR_BUTTON_GPIO, 0));
                    ESP_ERROR_CHECK(rtc_gpio_pulldown_en(PWR_BUTTON_GPIO)); // thả xuống nội bộ
                    ESP_ERROR_CHECK(rtc_gpio_pullup_dis(PWR_BUTTON_GPIO));
                    /* Tắt nguồn kích hoạt */
                    rtc_gpio_set_level(PWR_EN_GPIO, 0);
                    rtc_gpio_hold_dis(PWR_EN_GPIO);
                    
                    // Đảm bảo tất cả các thiết bị ngoại vi đã tắt
