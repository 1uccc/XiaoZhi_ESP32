...
// Lớp hiển thị LCD tùy chỉnh để thích ứng với màn hình tròn
...
private:
    ...
    // Chức năng ánh xạ âm lượng: ánh xạ âm lượng bên trong (0-80) sang âm lượng hiển thị (0-100%)
    int MapVolumeForDisplay(int internal_volume) {
        // Đảm bảo đầu vào nằm trong phạm vi hợp lệ
        if (internal_volume < 0) internal_volume = 0;
        if (internal_volume > 80) internal_volume = 80;
        
        // Bản đồ 0-80 đến 0-100
        // Công thức: Âm lượng hiển thị = (Âm lượng bên trong/80) * 100
    ...
    void InitializePowerSaveTimer() {
        ...
        if (cause == ESP_SLEEP_WAKEUP_EXT0) {
            ...
            if (long_press_occurred) {
                ...
                ESP_ERROR_CHECK(rtc_gpio_pullup_en(PWR_BUTTON_GPIO));  // Kéo lên bên trong
        ...
        // power_save_timer_ = PowerSaveTimer mới(-1, 6, 10);//kiểm tra
        ...
        power_save_timer_->OnShutdownRequest([this]() {
            ...
            esp_lcd_panel_disp_on_off(panel, false); // Tắt hiển thị
            esp_deep_sleep_start();
            #else
    ...
    void InitializeButtons() {
        ...
        // Kiểm tra trạng thái ban đầu của nút nguồn
        ESP_LOGI(TAG, "Power button initial state: %d", GpioManager::GetLevel(PWR_BUTTON_GPIO));

        // Logic tắt máy nhấn và hoạt động ở mức độ cao
        ...
        ESP_LOGI(TAG, "Power button long press detected (high-active)");
            ...
            // Xác nhận chống rung hiệu quả ở mức độ cao
            for (int i = 0; i < 5; i++) {
        ...
        // Bấm để chuyển trạng thái
        pwr_button_.OnClick([this]()
                            {
            // Nhận phiên bản và trạng thái ứng dụng hiện tại
            ...
            if (current_state == kDeviceStateIdle) {
                // Nếu hiện đang ở trạng thái chờ, hãy chuyển sang trạng thái nghe
                ESP_LOGI(TAG, "从待命状态切换到聆听状态");
                app.ToggleChatState(); // Chuyển sang trạng thái nghe
            } else if (current_state == kDeviceStateListening) {
                // Nếu hiện đang ở trạng thái nghe, hãy chuyển sang trạng thái chờ
                ESP_LOGI(TAG, "从聆听状态切换到待命状态");
                app.ToggleChatState(); // Chuyển sang trạng thái chờ
            } else if (current_state == kDeviceStateSpeaking) {
                // Nếu hiện đang nói, hãy ngừng nói và chuyển sang trạng thái chờ
                ESP_LOGI(TAG, "从说话状态切换到待命状态");
                app.ToggleChatState(); // ngừng nói chuyện
            } else {
                // Chỉ đánh thức thiết bị ở trạng thái khác
                ESP_LOGI(TAG, "唤醒设备");
                power_save_timer_->WakeUp();
            } });

        // Bấm nút nguồn ba lần: Đặt lại WiFi
        ...
        wifi_button.OnPressDown([this]()
           ...
           ESP_LOGI(TAG, "Volume up button pressed");
            ...
            int current_vol = codec->output_volume(); // Lấy khối lượng hiện tại thực tế
        ...
        void InitializeGC9301isplay()
        {
            ...
            // Khởi tạo chip điều khiển LCD 9309
