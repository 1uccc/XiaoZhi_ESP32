...
class PowerManager {
private:
    // Dải nguồn pin - điện trở chia điện áp là 2 x 100k
    ...
    inline static bool battery_update_paused_ = false;  // Cờ tĩnh: có nên tạm dừng cập nhật nguồn không

    adc_oneshot_unit_handle_t adc_handle_;

    void CheckBatteryStatus() {
      // Nếu quá trình cập nhật pin bị tạm dừng (đang thực hiện), hãy bỏ qua cập nhật
    ...
    void ReadBatteryAdcData() {
        ...
        // ESP_LOGI("PowerManager", "Giá trị ADC: %d trung bình: %ld công suất: %u%%", adc_value, Average_adc,
        // pin_level_);
    }
...
public:
    PowerManager(gpio_num_t charging_pin, adc_unit_t adc_unit = ADC_UNIT_2,
      ...
      if (charging_pin_ != GPIO_NUM_NC) {
        ...
        ESP_ERROR_CHECK(esp_timer_start_periodic(timer_handle_, 1000000));  // 1 giây
    ...
    // Tạm dừng/tiếp tục cập nhật nguồn (được sử dụng để chặn cập nhật khi các hành động được thực thi)
