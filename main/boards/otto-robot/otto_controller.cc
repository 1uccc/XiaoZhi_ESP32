...
private:
    ...
    struct OttoActionParams {
        ...
        int direction;
        int amount;
        char servo_sequence_json[512];  // Chuỗi JSON được sử dụng để lưu trữ chuỗi servo
    };
    ...
    enum ActionType {
        ...
        ACTION_RADIO_CALISTHENICS = 26,  // thể dục đài phát thanh
        ACTION_MAGIC_CIRCLE = 27,  // Sự kỳ diệu của tình yêu cứ quay vòng
        ACTION_UPDOWN = 8,
        ...
        ACTION_HAND_WAVE = 16,
        ACTION_WINDMILL = 20,  // cối xay gió lớn
        ACTION_TAKEOFF = 21,   // cởi
        ACTION_FITNESS = 22,   // sự thích hợp
        ACTION_GREETING = 23,  // chào
        ACTION_SHY = 24,        // xấu hổ
        ACTION_SHOWCASE = 28,   // thể hiện hành động
        ACTION_HOME = 17,
        ACTION_SERVO_SEQUENCE = 18,  // Trình tự servo (tự lập trình)
        ACTION_WHIRLWIND_LEG = 19    // cú đá lốc xoáy
    };
    ...
    static void ActionTask(void* arg) {
        ...
        while (true) {
            if (xQueueReceive(controller->action_queue_, &params, pdMS_TO_TICKS(1000)) == pdTRUE) {
                ...
                PowerManager::PauseBatteryUpdate();  // Tạm dừng cập nhật pin khi hành động bắt đầu
                controller->is_action_in_progress_ = true;
                if (params.action_type == ACTION_SERVO_SEQUENCE) {
                    // Thực thi chuỗi servo (tự lập trình) - chỉ hỗ trợ định dạng tên khóa ngắn
                    cJSON* json = cJSON_Parse(params.servo_sequence_json);
                    if (json != nullptr) {
                        ESP_LOGD(TAG, "JSON解析成功，长度=%d", strlen(params.servo_sequence_json));
                        // Sử dụng tên khóa ngắn "a" để biểu thị mảng hành động
                        ...
                        if (cJSON_IsArray(actions)) {
                            ...
                            // Nhận độ trễ sau khi hoàn thành việc thực hiện trình tự (tên viết tắt "d", tham số cấp cao nhất)
                            ...
                            // Khởi tạo vị trí servo hiện tại (được sử dụng để duy trì các vị trí servo không xác định)
                            int current_positions[SERVO_COUNT];
                            for (int j = 0; j < SERVO_COUNT; j++) {
                                current_positions[j] = 90;  // Vị trí ở giữa mặc định
                            }
                            ...
                            for (int i = 0; i < array_size; i++) {
                                ...
                                if (cJSON_IsObject(action_item)) {
                                    ...
                                    if (cJSON_IsObject(osc_item)) {
                                        ...
                                        // Đọc góc trung tâm (tên phím ngắn "o"), mặc định 90 độ (góc tuyệt đối 0-180 độ)
                                        for (int j = 0; j < SERVO_COUNT; j++) {
                                            center_angle[j] = 90;  // Góc trung tâm mặc định là 90 độ (vị trí chính giữa)
                                        }
                                        ...
                                        // Kiểm tra an toàn: Ngăn chặn chân trái và chân phải dao động cùng lúc (kiểm tra biên độ)
                                        ...
                                        if (left_leg_large && right_leg_large) {
                                            ...
                                            amplitude[RIGHT_LEG] = 0;  // Cấm dao động chân phải
                                        }
                                        if (left_foot_large && right_foot_large) {
                                            ESP_LOGW(TAG, "检测到左右脚同时大幅度振荡，限制右脚振幅");
                                            amplitude[RIGHT_FOOT] = 0;  // Cấm dao động chân phải
                                        }
                                        
                                        // Đọc độ lệch pha (tên viết tắt "ph", tính bằng độ, được chuyển đổi thành radian)
                                        ...
                                        // Thời gian đọc (tên khóa ngắn "p"), phạm vi 100-3000 mili giây
                                        cJSON* period_item = cJSON_GetObjectItem(osc_item, "p");
                                        if (cJSON_IsNumber(period_item)) {
                                            period = period_item->valueint;
                                            if (period < 100) period = 100;
                                            if (period > 3000) period = 3000;  // Như mô tả, giới hạn ở 3000 mili giây
                                        }
                                        
                                        // Số chu kỳ đọc (tên khóa ngắn "c"), phạm vi 0,1-20,0
                                        ...
                                        if (cJSON_IsNumber(steps_item)) {
                                            ...
                                            if (steps > 20.0) steps = 20.0;  // Theo mô tả, giới hạn 20,0
                                        }
                                        
                                        // Thực hiện dao động - sử dụng Thực thi2, căn giữa ở góc tuyệt đối
                                        ...
                                        // Cập nhật vị trí sau khi dao động (sử dụng center_angle làm vị trí cuối cùng)
                                        for (int j = 0; j < SERVO_COUNT; j++) {
                                            current_positions[j] = center_angle[j];
                                        }
                                    } else {
                                        // Chế độ di chuyển bình thường
                                        // Sao chép từ mảng vị trí hiện tại, giữ vị trí servo không xác định
                                        ...
                                        if (cJSON_IsObject(servos_item)) {
                                            ...
                                            for (int j = 0; j < SERVO_COUNT; j++) {
                                                ...
                                                if (cJSON_IsNumber(servo_value)) {
                                                    ...
                                                    // Giới hạn phạm vi vị trí ở 0-180 độ
                                        ...
                                        // Nhận tốc độ di chuyển (tên khóa ngắn "v", mặc định 1000 mili giây)
                                        int speed = 1000;
                                        cJSON* speed_item = cJSON_GetObjectItem(action_item, "v");
                                        if (cJSON_IsNumber(speed_item)) {
                                            speed = speed_item->valueint;
                                            if (speed < 100) speed = 100;  // Tối thiểu 100 mili giây
                                            if (speed > 3000) speed = 3000;  // Error 500 (Server Error)!!1500.That’s an error.There was an error. Please try again later.That’s all we know.
                                        }
                                        
                                        // Thực hiện chuyển động servo
                                        ...
                                        // Cập nhật mảng vị trí hiện tại cho hành động tiếp theo
                                        for (int j = 0; j < SERVO_COUNT; j++) {
                                            current_positions[j] = servo_target[j];
                                        }
                                    }
                                    
                                    // Nhận thời gian trễ sau hành động (tên khóa ngắn "d")
                                    int delay_after = 0;
                                    ...
                                    // Trì hoãn sau hành động (không trì hoãn sau hành động cuối cùng)
                            ...
                            // Độ trễ sau khi hoàn tất thực hiện trình tự (được sử dụng để tạm dừng giữa các trình tự)
                            if (sequence_delay > 0) {
                                // Kiểm tra xem có còn chuỗi nào trong hàng đợi được thực thi không
                    ...
                    // Thực hiện các hành động được xác định trước
                ...
                PowerManager::ResumeBatteryUpdate();  // Tiếp tục cập nhật pin khi hành động kết thúc
    ...
    void QueueServoSequence(const char* servo_sequence_json) {
        ...
        // Sao chép chuỗi JSON vào cấu trúc (độ dài giới hạn)
...
public:
    OttoController(const HardwareConfig& hw_config) {
        ...
        QueueAction(ACTION_HOME, 1, 1000, 1, 0);  // hướng = 1 có nghĩa là đặt lại tay
    ...
    void RegisterMcpTools() {
        ...
        // Công cụ hành động hợp nhất (tất cả các hành động ngoại trừ trình tự servo)
        mcp_server.AddTool("self.otto.action",
                           ...
                           [this](const PropertyList& properties) -> ReturnValue {
                               ...
                               // Tất cả các tham số đều có giá trị mặc định và có thể truy cập trực tiếp
        ...
        // Công cụ tuần tự servo (hỗ trợ gửi phân đoạn, gửi từng chuỗi một và tự động xếp hàng để thực hiện)
        mcp_server.AddTool(
            ...
            [this](const PropertyList& properties) -> ReturnValue {
                ...
                // Kiểm tra xem đó có phải là đối tượng JSON không (có thể ở định dạng chuỗi hoặc đối tượng được phân tích cú pháp)
                // Nếu chuỗi là một chuỗi JSON, hãy sử dụng nó trực tiếp; if it is an object string, you also need to use it.
        ...
        mcp_server.AddTool("self.otto.stop", "立即停止所有动作并复位", PropertyList(),
                           [this](const PropertyList& properties) -> ReturnValue {
                               ...
                               PowerManager::ResumeBatteryUpdate();  // Tiếp tục cập nhật pin khi dừng hành động
        ...
        mcp_server.AddTool(
            ...
            [this](const PropertyList& properties) -> ReturnValue {
                ...
                // Nhận tất cả các giá trị cắt hiện tại
                ...
                // Cập nhật giá trị cắt của servo được chỉ định
