// --------------------------------------------------------------
// -- Dao động.pde
// - Tạo dao động hình sin trong động cơ servo
// --------------------------------------------------------------
// Error 500 (Server Error)!!1500.That’s an error.There was an error. Please try again later.That’s all we know.
// -- (c) txp666 cho Esp32, 202503
// -- Giấy phép GPL
// --------------------------------------------------------------
...
#define SERVO_TIMEBASE_RESOLUTION_HZ 1000000  // 1 MHz, 1us mỗi tích tắc
#define SERVO_TIMEBASE_PERIOD 20000           // 20000 tích tắc, 20ms
...
private:
    ...
    int offset_;              // -- Bù đắp (độ)
    unsigned int period_;     // -- Khoảng thời gian (mili giây)
    double phase0_;           // -- Pha (radian)

    // -- Biến nội bộ
    int pos_;                       // -- Vị trí servo hiện tại
    int pin_;                       // -- Ghim nơi kết nối servo
    int trim_;                      // -- Hiệu chuẩn bù đắp
    double phase_;                  // -- Giai đoạn hiện tại
    double inc_;                    // -- Tăng pha
    double number_samples_;         // -- Số lượng mẫu
    unsigned int sampling_period_;  // -- thời gian lấy mẫu (ms)

    long previous_millis_;
    long current_millis_;

    // - Chế độ dao động. Nếu đúng, servo sẽ dừng
    bool stop_;
