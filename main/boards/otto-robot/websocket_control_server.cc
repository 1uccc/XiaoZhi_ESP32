...
esp_err_t WebSocketControlServer::ws_handler(httpd_req_t *req) {
    ...
    /* Đặt max_len = 0 để lấy khung len */
    ...
    if (ws_pkt.len) {
        /* ws_pkt.len + 1 dành cho việc chấm dứt NULL vì chúng tôi đang mong đợi một chuỗi */
        ...
        ws_pkt.payload = buf;
        /* Đặt max_len = ws_pkt.len để lấy tải trọng khung */
...
void WebSocketControlServer::HandleMessage(httpd_req_t *req, const char* data, size_t len) {
    ...
    // 2. Định dạng đơn giản: trực tiếp đối tượng tải trọng MCP
