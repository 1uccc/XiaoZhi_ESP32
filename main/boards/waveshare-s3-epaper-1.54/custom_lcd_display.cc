...
CustomLcdDisplay::CustomLcdDisplay(esp_lcd_panel_io_handle_t panel_io, esp_lcd_panel_handle_t panel, 
    ...
    assert(buffer);
    display_ = lv_display_create(width, height); /* Khởi tạo cơ bản với độ phân giải ngang và dọc (pixel) */
    ...
    EPD_Init_Partial(); // Khởi tạo làm mới một phần
...
void CustomLcdDisplay::spi_port_init() {
    ...
    devcfg.clock_speed_hz                = 40 * 1000 * 1000; // Đồng hồ ra ở 10 MHz
    devcfg.mode                          = 0;                // Chế độ SPI 0
    devcfg.queue_size                    = 7;                // Chúng tôi muốn có thể xếp hàng 7 giao dịch cùng một lúc
...
void CustomLcdDisplay::read_busy() {
    ...
    while (gpio_get_level((gpio_num_t) busy) == 1) {
        vTaskDelay(pdMS_TO_TICKS(5)); // THẤP: nhàn rỗi, CAO: bận
    }
}
...
void CustomLcdDisplay::SPI_SendByte(uint8_t data) {
    ...
    t.length    = 8;
    t.tx_buffer = &data;
    ret         = spi_device_polling_transmit(spi, &t); // Truyền đi!
    assert(ret == ESP_OK);                              // Đáng lẽ không có vấn đề gì.
}
...
void CustomLcdDisplay::EPD_Init() {
    ...
    EPD_SendCommand(0x01); // Kiểm soát đầu ra trình điều khiển
    EPD_SendData(0xC7);
    EPD_SendData(0x00);
    EPD_SendData(0x01);

    EPD_SendCommand(0x11); // chế độ nhập dữ liệu
    EPD_SendData(0x01);
    ...
    EPD_SendCommand(0x22); // Tải nhiệt độ và cài đặt dạng sóng.
    EPD_SendData(0XB1);
    EPD_SendCommand(0x20);
