...
K10AudioCodec::K10AudioCodec(void* i2c_master_handle, int input_sample_rate, int output_sample_rate,
    ...
    duplex_ = true; // Cho dù song công
    input_reference_ = input_reference; // Có nên sử dụng đầu vào tham chiếu để khử tiếng vang hay không
    input_channels_ = input_reference_ ? 2 : 1; // Số kênh đầu vào
    input_sample_rate_ = input_sample_rate;
    output_sample_rate_ = output_sample_rate;

    CreateDuplexChannels(mclk, bclk, ws, dout, din);

    // Khởi tạo giao diện liên quan: data_if, ctrl_if và gpio_if
...
void K10AudioCodec::EnableInput(bool enable) {
    ...
    if (enable) {
        ...
        ESP_ERROR_CHECK(esp_codec_dev_set_in_gain(input_dev_, 37.5)); // Tăng micrô giải quyết vấn đề thu âm quá nhỏ
    } else {
        ESP_ERROR_CHECK(esp_codec_dev_close(input_dev_));
    }
...
int K10AudioCodec::Write(const int16_t* data, int samples) {
    if (output_enabled_) {
        std::vector<int32_t> buffer(samples * 2);  // Phân bổ bộ đệm cho các mẫu 2x

        // Áp dụng điều chỉnh âm lượng (giống như trước)
        ...
        for (int i = 0; i < samples; i++) {
            ...
            // Lặp lại từng mẫu để phát lại chậm (giả sử âm thanh đơn âm)
