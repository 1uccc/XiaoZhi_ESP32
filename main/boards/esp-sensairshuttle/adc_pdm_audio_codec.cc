...
/* *
 * @brief Cấu trúc cấu hình Mono Duplex I2S
 *
 * Cấu hình này được sử dụng theo mặc định trong bsp_audio_init() */
...
AdcPdmAudioCodec::AdcPdmAudioCodec(int input_sample_rate, int output_sample_rate,
    ...
    chan_cfg.auto_clear = true; // Tự động xóa dữ liệu cũ trong bộ đệm DMA
    ...
    if(pdm_speak_n != GPIO_NUM_NC){
        ...
        esp_rom_gpio_connect_out_signal(pdm_speak_n, I2SO_SD_OUT_IDX, 1, 0); // Đảo ngược tín hiệu SD OUT đầu ra
        gpio_set_drive_capability(pdm_speak_n, GPIO_DRIVE_CAP_0);
    }

    // Khởi tạo bộ đếm thời gian đầu ra
...
void AdcPdmAudioCodec::EnableOutput(bool enable) {
    ...
    if (enable) {
        ...
        // Buộc đồng hồ PDM TX được cấu hình lại theo cấu hình bo mạch, ghi đè up_sample_fs mặc định của thư viện bên thứ ba trong set_fmt
        // Nếu kênh đã được bật, trước tiên hãy tắt kênh đó, định cấu hình lại và cuối cùng bật kênh đó.
        ...
        // Bắt đầu hẹn giờ khi đầu ra được bật
        if (output_timer_) {
            esp_timer_start_once(output_timer_, TIMER_TIMEOUT_US);
        }

    } else {
        // Dừng hẹn giờ khi đầu ra bị tắt
        if (output_timer_) {
            esp_timer_stop(output_timer_);
        }
        if(pa_ctrl_pin_ != GPIO_NUM_NC){
...
int AdcPdmAudioCodec::Write(const int16_t* data, int samples) {
    if (output_enabled_) {
        ...
        // Đặt lại bộ đếm thời gian đầu ra
        if (output_timer_) {
            esp_timer_stop(output_timer_);
            esp_timer_start_once(output_timer_, TIMER_TIMEOUT_US);
        }
...
// Thực hiện chức năng gọi lại hẹn giờ
