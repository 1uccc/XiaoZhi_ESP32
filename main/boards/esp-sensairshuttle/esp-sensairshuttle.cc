...
static const ili9341_lcd_init_cmd_t vendor_specific_init[] = {
    ...
    {0x11, NULL, 0, 120},                                          // Ngủ ngoài
    {0x36, (uint8_t []){0x00}, 1, 0},                              // Kiểm soát truy cập dữ liệu bộ nhớ
    {0x3A, (uint8_t []){0x05}, 1, 0},                              // Định dạng pixel giao diện (16-bit)
    {0xB2, (uint8_t []){0x0C, 0x0C, 0x00, 0x33, 0x33}, 5, 0},      // Thiết lập hiên nhà
    {0xB7, (uint8_t []){0x05}, 1, 0},                              // Kiểm soát cổng
    {0xBB, (uint8_t []){0x21}, 1, 0},                              // Cài đặt VCOM
    {0xC0, (uint8_t []){0x2C}, 1, 0},                              // Kiểm soát LCM
    {0xC2, (uint8_t []){0x01}, 1, 0},                              // Kích hoạt lệnh VDV và VRH
    {0xC3, (uint8_t []){0x15}, 1, 0},                              // Bộ hàng đầu
    {0xC6, (uint8_t []){0x0F}, 1, 0},                              // Kiểm soát tốc độ khung hình
    {0xD0, (uint8_t []){0xA7}, 1, 0},                              // Điều khiển nguồn 1
    {0xD0, (uint8_t []){0xA4, 0xA1}, 2, 0},                        // Điều khiển nguồn 1
    {0xD6, (uint8_t []){0xA1}, 1, 0},                              // Cổng đầu ra GND ở chế độ ngủ
    {
        0xE0, (uint8_t [])
        {
    ...
    {0xE4, (uint8_t []){0x23, 0x00, 0x00}, 3, 0},                  // Kiểm soát vị trí cổng
    {0x21, NULL, 0, 0},                                            // Hiển thị đảo ngược bật
    {0x29, NULL, 0, 0},                                            // Hiển thị trên
    {0x2C, NULL, 0, 0},                                            // Ghi nhớ
};
...
public:
    ...
    TouchEvent CheckTouchEvent()
    {
        ...
        if (is_touched && !was_touched_) {
            // Sự kiện nhấn (chuyển từ không chạm sang chạm)
            press_count_++;
            event = TOUCH_PRESS;
            ESP_LOGI(TAG, "TOUCH PRESS - count: %d, x: %d, y: %d", press_count_, tp_.x, tp_.y);
        } else if (!is_touched && was_touched_) {
            // Sự kiện phát hành (chuyển từ trạng thái chạm sang không chạm)
            event = TOUCH_RELEASE;
...
private:
    ...
    static void touch_event_task(void* arg)
    {
        ...
        while (true) {
            ...
            vTaskDelay(pdMS_TO_TICKS(50)); // Thăm dò cứ sau 50ms
        }
    }
