...
private:
    ...
    void InitializeTca9554(void) {
        ...
        // ret = Esp_io_expander_set_dir(io_expander, IO_EXPANDER_PIN_NUM_0 | IO_EXPANDER_PIN_NUM_1, IO_EXPANDER_INPUT); // Đặt chế độ pin EXIO0 và EXIO1 làm đầu vào
        // ret = Esp_io_expander_get_level(io_expander, IO_EXPANDER_PIN_NUM_0 | IO_EXPANDER_PIN_NUM_1, &input_level_mask); // Lấy trạng thái cấp độ của các chân EXIO0 và EXIO1 và lưu chúng vào input_level_mask

        // ret = Esp_io_expander_set_dir(io_expander, IO_EXPANDER_PIN_NUM_2 | IO_EXPANDER_PIN_NUM_3, IO_EXPANDER_OUTPUT); // Đặt chế độ pin EXIO2 và EXIO3 thành đầu ra
        // ret = Esp_io_expander_set_level(io_expander, IO_EXPANDER_PIN_NUM_2 | IO_EXPANDER_PIN_NUM_3, 1); // Đặt mức pin thành 1
        // ret = Esp_io_expander_print_state(io_expander); // In trạng thái ghim

        ret = esp_io_expander_set_dir(io_expander, IO_EXPANDER_PIN_NUM_0 | IO_EXPANDER_PIN_NUM_1, IO_EXPANDER_OUTPUT);                 // Đặt chân chế độ EXIO0 và EXIO1 thành đầu ra
        ESP_ERROR_CHECK(ret);
        ret = esp_io_expander_set_level(io_expander, IO_EXPANDER_PIN_NUM_0 | IO_EXPANDER_PIN_NUM_1, 1);                                // Đặt lại LCD và TouchPad
        ESP_ERROR_CHECK(ret);
        vTaskDelay(pdMS_TO_TICKS(300));
        ret = esp_io_expander_set_level(io_expander, IO_EXPANDER_PIN_NUM_0 | IO_EXPANDER_PIN_NUM_1, 0);                                // Đặt lại LCD và TouchPad
        ESP_ERROR_CHECK(ret);
        vTaskDelay(pdMS_TO_TICKS(300));
        ret = esp_io_expander_set_level(io_expander, IO_EXPANDER_PIN_NUM_0 | IO_EXPANDER_PIN_NUM_1, 1);                                // Đặt lại LCD và TouchPad
        ESP_ERROR_CHECK(ret);
    ...
    void Initializest77916Display() {
        ...
        lcd_cmd &= 0xff;
        lcd_cmd <<= 8;
        lcd_cmd |= LCD_OPCODE_READ_CMD << 24;  // Sử dụng opcode đọc thay vì viết
        ...
        // Kiểm tra giá trị đăng ký và cấu hình cho phù hợp
        if (register_data[0] == 0x00 && register_data[1] == 0x7F && register_data[2] == 0x7F && register_data[3] == 0x7F) {
            // Xử lý trường hợp dữ liệu đăng ký khớp với mẫu này
            ...
            // Cung cấp các lệnh khởi tạo dành riêng cho nhà cung cấp nếu dữ liệu đăng ký khớp với mẫu này
        ...
        const esp_lcd_panel_dev_config_t panel_config = {
            ...
            .rgb_ele_order = LCD_RGB_ELEMENT_ORDER_RGB,     // Thực hiện bằng lệnh LCD `36h`
            .bits_per_pixel = QSPI_LCD_BIT_PER_PIXEL,    // Thực hiện bằng lệnh LCD `3Ah` (18/16)
    ...
    void InitializeButtonsCustom() {
        ...
        // gpio_set_level(PWR_Control_PIN, sai);
