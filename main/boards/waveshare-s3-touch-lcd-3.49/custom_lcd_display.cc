...
void CustomLcdDisplay::lvgl_port_flush_callback(lv_display_t *drv, const lv_area_t *area, uint8_t *color_map) {
    ...
    if(rotation != LV_DISPLAY_ROTATION_0) {
        ...
        /* Tính toán vị trí của khu vực quay */
        rotated_area = *area;
        lv_display_rotate_area(drv, &rotated_area);
        /* Tính toán bước nguồn (byte trên một dòng) từ chiều rộng của khu vực */
        uint32_t src_stride = lv_draw_buf_width_to_stride(lv_area_get_width(area), cf);
        /* Tính cả bước đi của khu vực đích (xoay) */
        uint32_t dest_stride = lv_draw_buf_width_to_stride(lv_area_get_width(&rotated_area), cf);
        /* Có bộ đệm để lưu trữ vùng đã xoay và thực hiện thao tác xoay */

        int32_t src_w = lv_area_get_width(area);
        int32_t src_h = lv_area_get_height(area);
        lv_draw_sw_rotate(color_map, dest_map, src_w, src_h, src_stride, dest_stride, rotation, cf);
        /* Sử dụng vùng xoay và vùng đệm được xoay kể từ bây giờ */
        area = &rotated_area;
    }
#endif
