...
// Chỉ dùng để đánh thức giấc ngủ sâu bằng cử chỉ cổ tay
...
private:
    ...
    static constexpr int kDeepSleepTimeoutSeconds = 10 * 60; // 10 phút
...
public:
    ...
    virtual bool GetBatteryLevel(int& level, bool& charging, bool& discharging) override {
        ...
        if (adc_calibration_lock_) {
            ...
            voltage = voltage * 3 / 2;  // bù cho bộ chia điện áp
        ...
        // Tính phần trăm mức pin
        level = (voltage - EMPTY_BATTERY_VOLTAGE) * 100 / (FULL_BATTERY_VOLTAGE - EMPTY_BATTERY_VOLTAGE);

        // ESP-Spot không hỗ trợ phát hiện sạc nên chúng tôi sử dụng MCU_VCC_CTL để xác định trạng thái sạc
