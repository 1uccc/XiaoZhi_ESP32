...
private:
    ...
    void InitializePowerSaveTimer() {
        // Tính năng phát hiện từ đánh thức sẽ bị tắt ở chế độ ngủ nhẹ
        sleep_timer_ = new SleepTimer(30);
        sleep_timer_->OnEnterLightSleepMode([this]() {
            ESP_LOGI(TAG, "Enabling sleep mode");
            // Hiển thị màn hình chờ
            GetDisplay()->SetPowerSaveMode(true);
            // Bật chế độ ngủ và ngủ sau 1 giây sau khi DTR được đặt ở mức cao
            modem_->SetSleepMode(true, 1);
            // Đặt chân DTR lên cao để modem chuyển sang chế độ ngủ
            modem_->GetAtUart()->SetDtrPin(true);
        });
        sleep_timer_->OnExitLightSleepMode([this]() {
            // Đặt chân DTR ở mức thấp để modem thức dậy
