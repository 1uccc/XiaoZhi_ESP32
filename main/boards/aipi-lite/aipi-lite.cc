...
class AIPILite : public WifiBoard {
   private:
    ...
    void InitializeButtons() {
        ...
        // Đặt sự kiện nhấn và giữ nút nguồn (vào trực tiếp chế độ phân phối mạng)
        boot_button_.OnLongPress([this]() {
            // Hẹn giờ bật chế độ tiết kiệm điện
            power_save_timer_->WakeUp();
            // Nhận phiên bản ứng dụng
            auto& app = Application::GetInstance();

            // Vào chế độ mạng phân phối
            app.SetDeviceState(kDeviceStateWifiConfiguring);

            // Đặt lại cấu hình WiFi để đảm bảo vào chế độ phân phối
            EnterWifiConfigMode();
        });
