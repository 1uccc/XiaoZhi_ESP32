...
class Hu087Board : public WifiBoard {
    ...
    void initializeAmpCtrl() {
        gpio_config_t io_conf = {
            .pin_bit_mask = (1ULL << AUDIO_I2S_SPK_GPIO_CTLR),  // Chọn GPIO 2
            .mode = GPIO_MODE_OUTPUT,                           // Đặt làm đầu ra
            .pull_up_en = GPIO_PULLUP_ENABLE,                   // Vô hiệu hóa kéo lên
            .pull_down_en = GPIO_PULLDOWN_DISABLE,              // Vô hiệu hóa kéo xuống
            .intr_type = GPIO_INTR_DISABLE  // Tắt ngắt
        };
        gpio_config(&io_conf);
    ...
    void InitializeButtons() {
        ...
        touch_button_.OnLongPress([this]() {
            ...
            if (volume > 100) {
                volume = 100;
            }  // Cần triển khai logic để giảm âm lượng
 ...
 public:
    Hu087Board() : touch_button_(TOUCH_BUTTON_GPIO) {
        ...
        InitializeButtons();
        initializeAmpCtrl();  // Có thể điều khiển amp ctrl pin thông qua giọng nói
                              // tôi đoán là phát hiện
    }
