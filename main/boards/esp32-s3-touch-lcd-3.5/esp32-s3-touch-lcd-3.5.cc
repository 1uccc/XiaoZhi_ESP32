...
class Pmic : public Axp2101 {
    public:
        Pmic(i2c_master_bus_handle_t i2c_bus, uint8_t addr) : Axp2101(i2c_bus, addr) {
            ...
            // Vô hiệu hóa tất cả các DC trừ DC1
            WriteReg(0x80, 0x01);
            // Vô hiệu hóa tất cả LDO
            WriteReg(0x90, 0x00);
            WriteReg(0x91, 0x00);
            ...
            // Đặt ALDO1 thành 3,3V
            WriteReg(0x92, (3300 - 500) / 100);

            WriteReg(0x96, (1500 - 500) / 100);
            WriteReg(0x97, (2800 - 500) / 100);
    
            // Kích hoạt ALDO1 BLDO1 BLDO2
            WriteReg(0x90, 0x31);
        
            WriteReg(0x64, 0x02); // Cài đặt điện áp bộ sạc CV thành 4.1V
            
            WriteReg(0x61, 0x02); // đặt dòng nạp trước pin chính thành 50mA
            WriteReg(0x62, 0x08); // đặt dòng sạc pin chính thành 400mA ( 0x08-200mA, 0x09-300mA, 0x0A-400mA )
            WriteReg(0x63, 0x01); // đặt dòng sạc pin chính thành 25mA
        }
    };
...
typedef struct {
    const st7796_lcd_init_cmd_t *init_cmds;     /* !< Con trỏ tới mảng lệnh khởi tạo. Đặt thành NULL nếu sử dụng các lệnh mặc định.
...
public:
    CustomBoard() :
        ...
        // Giải quyết vấn đề màn hình đen khi khởi động
