...
void Tcamerapluss3AudioCodec::CreateVoiceHardware(gpio_num_t mic_bclk, gpio_num_t mic_ws, gpio_num_t mic_data,
    ...
    mic_chan_config.auto_clear = true; // Tự động xóa dữ liệu cũ trong bộ đệm DMA
    i2s_chan_config_t spkr_chan_config = I2S_CHANNEL_DEFAULT_CONFIG(I2S_NUM_1, I2S_ROLE_MASTER);
    spkr_chan_config.auto_clear = true; // Tự động xóa dữ liệu cũ trong bộ đệm DMA
    ...
    i2s_std_config_t mic_config = {
        ...
        .gpio_cfg = {
            ...
            .invert_flags = {
                .mclk_inv = false,
                .bclk_inv = false,
                .ws_inv = true // Kênh bên phải mặc định
            }
        }
    };
    ...
    i2s_pdm_rx_config_t mic_config = {
        ...
        /* Độ rộng bit dữ liệu của chế độ PDM được cố định là 16 */
...
int Tcamerapluss3AudioCodec::Read(int16_t *dest, int samples) {
    if (input_enabled_) {
        ...
        // Âm lượng thu của micrô được khuếch đại 20 lần (giới hạn ở phạm vi int16_t để tránh tràn)
        int16_t *ptr = dest;
