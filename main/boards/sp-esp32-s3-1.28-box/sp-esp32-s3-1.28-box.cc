...
public:
    CustomLcdDisplay(esp_lcd_panel_io_handle_t io_handle,
        ...
        // Vì màn hình tròn nên thanh trạng thái cần thêm phần đệm trái và phải.
...
private:
    ...
    void InitializePowerSaveTimer() {
        ...
        power_save_timer_->OnShutdownRequest([this]() {
            ...
            // Kích hoạt chức năng giữ để đảm bảo mức độ ổn định trong khi ngủ
    ...
    static void touchpad_timer_callback(void* arg) {
        ...
        const int64_t TOUCH_THRESHOLD_MS = 500;  // Ngưỡng thời lượng chạm, vượt quá 500ms được coi là nhấn lâu

        board->cst816d_->UpdateTouchPoint();
        auto touch_point = board->cst816d_->GetTouchPoint();

        // Phát hiện khởi động bằng cảm ứng
        if (touch_point.num > 0 && !was_touched) {
            was_touched = true;
            touch_start_time = esp_timer_get_time() / 1000; // Chuyển đổi sang mili giây
        }
        // Phát hiện nhả cảm ứng
        else if (touch_point.num == 0 && was_touched) {
            was_touched = false;
            int64_t touch_duration = (esp_timer_get_time() / 1000) - touch_start_time;

            // Chỉ được kích hoạt bằng cú chạm ngắn
            if (touch_duration < TOUCH_THRESHOLD_MS) {
                auto& app = Application::GetInstance();
                // Trong khi khởi động (trước khi kết nối), nhấn cảm ứng sẽ chuyển sang chế độ cấu hình Wi-Fi mà không cần khởi động lại
    ...
    void InitializeCst816DTouchPad() {
        ...
        if (!touch_available) {
            ...
            // Nhả cảm ứng I2C để tránh lặp lại lỗi khi không có thiết bị
        ...
        // Tạo bộ đếm thời gian, khoảng thời gian 10ms
    ...
    void InitializeGc9a01Display() {
        ...
        panel_config.reset_gpio_num = DISPLAY_SPI_RESET_PIN;    // Đặt thành -1 nếu không sử dụng
    ...
    void InitializeButtons() {
        boot_button_.OnClick([this]() {
            auto& app = Application::GetInstance();
            // Trong khi khởi động (trước khi kết nối), nhấn nút BOOT sẽ vào chế độ cấu hình Wi-Fi mà không cần khởi động lại
            if (app.GetDeviceState() == kDeviceStateStarting) {
                EnterWifiConfigMode();
                return;
            }
...
public:
    Spotpear_ESP32_S3_1_28_BOX() : boot_button_(BOOT_BUTTON_GPIO) {
        // Đầu tiên hãy khởi tạo cảm ứng I2C và phát hiện/khởi tạo cảm ứng (bỏ qua nếu không có cảm ứng)
        InitializeCodecI2c_Touch();
        InitializeCst816DTouchPad();

        // Khởi tạo âm thanh I2C
        InitializeCodecI2c();

        // Mối tương quan hiển thị được thiết lập đầu tiên
        InitializeSpi();
        ...
        // Khởi tạo logic tiết kiệm năng lượng sau khi màn hình và đèn nền khả dụng để tránh con trỏ null
