...
// Số lần thử lại tối đa để phát hiện modem
static constexpr int MODEM_DETECT_MAX_RETRIES = 30;
// Số lần thử lại tối đa để đăng ký mạng
...
void Ml307Board::NetworkTask() {
    // Thông báo đã bắt đầu phát hiện modem
    OnNetworkEvent(NetworkEvent::ModemDetecting);

    // Cố gắng phát hiện modem có giới hạn thử lại
    ...
    // Thiết lập gọi lại thay đổi trạng thái mạng
    // Lưu ý: Đừng gọi GetCarrierName() tại đây vì nó sẽ gửi lệnh AT và sẽ chặn NhậnTask
    ...
    // Thông báo đăng ký mạng đã bắt đầu
    OnNetworkEvent(NetworkEvent::Connecting);

    // Đợi mạng sẵn sàng với giới hạn thử lại
    int reg_retries = 0;
...
void Ml307Board::StartNetwork() {
    // Tạo tác vụ khởi tạo mạng và quay trở lại ngay lập tức
...
void Ml307Board::SetPowerSaveLevel(PowerSaveLevel level) {
    // VIỆC CẦN LÀM: Triển khai mức tiết kiệm năng lượng cho ML307
    (void)level;
}

std::string Ml307Board::GetDeviceStatusJson() {
    /* * Trả về trạng thái thiết bị JSON
     *
     ...
     * "mạng": {
     * "loại": "di động",
     * "nhà cung cấp dịch vụ": "CHINA MOBILE",
     * "csq": 10
     * }
     * } */
    ...
    if (display && display->height() > 64) { // Chỉ dành cho màn hình LCD
