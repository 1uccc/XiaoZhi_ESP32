...
int Sy6970::GetBatteryLevel() {
    int level = 0;
    // Điện áp thấp nhất mà pin có thể xả
    int battery_minimum_voltage = 3200;
    int battery_voltage = GetBatteryVoltage();
    int charge_voltage_limit = GetChargeTargetVoltage();
    // ESP_LOGI(TAG, "điện áp pin: %d, giới hạn điện áp sạc: %d", điện áp pin, giới hạn điện áp sạc);
    ...
    // Trạng thái sạc được đọc khi pin không được kết nối không ổn định và điện áp pin đôi khi vượt quá giới hạn sạc_điện áp.
    if (level > 100) {
        level = 100;
    }
    return level;
