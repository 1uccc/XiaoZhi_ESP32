...
Esp32Camera::Esp32Camera(const camera_config_t &config) {
    ...
    if (s) {
        if (s->id.PID == GC0308_PID) {
            s->set_hmirror(s, 0); // Điều khiển gương camera: 1 cho gương, 0 cho bình thường
        }
...
bool Esp32Camera::Capture() {
    ...
    // Nhận khung hình mới nhất, loại bỏ khung hình cũ để có hiệu suất theo thời gian thực
    for (int i = 0; i < 2; i++) {
        if (current_fb_) {
            esp_camera_fb_return(current_fb_);
        }
    ...
    // Thực hiện hoán đổi byte cho định dạng RGB565 và chuẩn bị hình ảnh xem trước
    if (current_fb_->format == PIXFORMAT_RGB565) {
        ...
        for (size_t i = 0; i < pixel_count; i++) {
            // Sao chép dữ liệu từ bộ đệm trình điều khiển sang bộ đệm xem trước bằng cách hoán đổi byte
        ...
        // Xem trước định dạng JPEG thường yêu cầu giải mã, bỏ qua hiển thị xem trước ngay bây giờ, chỉ cần đăng nhập
    ...
    encoder_thread_ = std::thread([this, jpeg_queue]() {
        ...
        switch (current_fb_->format) {
            ...
            case PIXFORMAT_YUV422:
                enc_fmt = V4L2_PIX_FMT_YUYV;  // YUV422 thực sự là định dạng YUYV
                break;
