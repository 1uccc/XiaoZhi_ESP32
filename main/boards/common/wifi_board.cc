...
WifiBoard::WifiBoard() {
    // Tạo bộ đếm thời gian chờ kết nối
...
void WifiBoard::StartNetwork() {
    ...
    // Đặt gọi lại sự kiện hợp nhất - chuyển tiếp tới NetworkEvent với dữ liệu SSID
    ...
    // Hãy thử kết nối hoặc vào chế độ cấu hình
    TryWifiConnect();
}
...
void WifiBoard::TryWifiConnect() {
    ...
    if (have_ssid) {
        // Bắt đầu thử kết nối khi hết thời gian chờ
        ESP_LOGI(TAG, "Starting WiFi connection attempt");
        esp_timer_start_once(connect_timer_, CONNECT_TIMEOUT_SEC * 1000000ULL);
        WifiManager::GetInstance().StartStation();
    } else {
        // Chưa có cấu hình SSID, hãy vào chế độ cấu hình
        // Đợi phiên bản bảng được hiển thị
...
void WifiBoard::OnNetworkEvent(NetworkEvent event, const std::string& data) {
    switch (event) {
        case NetworkEvent::Connected:
            // Dừng hẹn giờ hết thời gian chờ
            esp_timer_stop(connect_timer_);
#ifdef CONFIG_USE_ESP_BLUFI_WIFI_PROVISIONING
            // đảm bảo tài nguyên blufi đã được phát hành
        ...
        case NetworkEvent::WifiConfigModeExit:
            ...
            in_config_mode_ = false;
            // Hãy thử kết nối với thông tin đăng nhập mới
            TryWifiConnect();
            break;
        default:
            break;
    ...
    // Thông báo gọi lại bên ngoài nếu được đặt
...
void WifiBoard::StartWifiConfigMode() {
    in_config_mode_ = true;
    // Chuyển sang trạng thái cấu hình wifi
    ...
    // Hiển thị lời nhắc cấu hình sau một khoảng thời gian trễ ngắn
    ...
    // khởi tạo giao thức Esp-blufi
    blufi.init();
#endif
#if CONFIG_USE_ACOUSTIC_WIFI_PROVISIONING
    // Bắt đầu nhiệm vụ cung cấp âm thanh
...
void WifiBoard::EnterWifiConfigMode() {
    ...
    if (state == kDeviceStateSpeaking || state == kDeviceStateListening || state == kDeviceStateIdle) {
        // Đặt lại giao thức (đóng kênh âm thanh, đặt lại giao thức)
        Application::GetInstance().ResetProtocol();

        xTaskCreate([](void* arg) {
            auto* board = static_cast<WifiBoard*>(arg);

            // Đợi 1 giây để cuộc nói chuyện kết thúc một cách duyên dáng
            vTaskDelay(pdMS_TO_TICKS(1000));

            // Dừng mọi nỗ lực kết nối đang diễn ra
