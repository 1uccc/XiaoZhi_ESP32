...
public:
    Pi4ioe(i2c_master_bus_handle_t i2c_bus, uint8_t addr) : I2cDevice(i2c_bus, addr) {
        WriteReg(PI4IOE_REG_IO_PP, 0x00); // Đặt ở mức trở kháng cao
        WriteReg(PI4IOE_REG_IO_PULLUP, 0xFF); // Kích hoạt tính năng kéo lên
        WriteReg(PI4IOE_REG_IO_DIR, 0x6E); // Đặt đầu vào=0, đầu ra=1
        WriteReg(PI4IOE_REG_IO_OUT, 0xFF); // Đặt đầu ra thành 1
    }
...
public:
    Lp5562(i2c_master_bus_handle_t i2c_bus, uint8_t addr) : I2cDevice(i2c_bus, addr) {
        WriteReg(0x00, 0B01000000); // Đặt chip_en thành 1
        WriteReg(0x08, 0B00000001); // Kích hoạt đồng hồ nội bộ
        WriteReg(0x70, 0B00000000); // Định cấu hình tất cả các đầu ra LED được điều khiển từ các thanh ghi I2C

        // Tần số xung nhịp 558 Hz
...
private:
    ...
    void InitializeGc9107Display() {
        ...
        panel_config.reset_gpio_num = GPIO_NUM_48; // Đặt thành -1 nếu không sử dụng
        panel_config.rgb_endian = LCD_RGB_ENDIAN_BGR;
        panel_config.bits_per_pixel = 16; // Thực hiện bằng lệnh LCD `3Ah` (18/16)
