...
private:
    ...
    void InitializeI2c() {
        ...
        if (xl9555_in_->GetPingState(0x0020) == 1) {
            es8311_detected_ = true;    /* Cờ thiết bị âm thanh, khi SPK_CTRL_IO ở mức cao, cờ được đặt thành 1 và được xác định là ES8311 */
        } else {
            es8311_detected_ = false;    /* Bit cờ của thiết bị âm thanh, khi SPK_CTRL_IO ở mức thấp, bit cờ là 0 và được xác định là NS4168 */
        }
...
public:
    ...
    virtual AudioCodec* GetAudioCodec() override {
        /* Khởi tạo codec dựa trên kết quả phát hiện */
        if (es8311_detected_) {
            /* Sử dụng trình điều khiển ES8311 */
