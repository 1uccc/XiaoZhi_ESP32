...
BoxAudioCodecLite::BoxAudioCodecLite(void* i2c_master_handle, int input_sample_rate, int output_sample_rate,
    ...
    input_channels_ = 2 + input_reference_; // Số kênh đầu vào
...
void BoxAudioCodecLite::EnableInput(bool enable) {
    ...
    if (enable) {
        ...
        // Tăng micrô giải quyết vấn đề thu âm quá nhỏ
...
int BoxAudioCodecLite::Write(const int16_t* data, int samples) {
    if (output_enabled_) {
        ...
        if (input_reference_) { // Bo mạch không hỗ trợ chức năng khử tiếng vang bằng phần cứng và sử dụng bộ đệm phát lại được lưu trong bộ nhớ đệm để đạt được chức năng khử tiếng vang.
            if (write_pos_ - read_pos_ + samples > ref_buffer_.size()) { 
                assert(ref_buffer_.size() >= samples);
                // Ghi tràn, chỉ giữ dữ liệu mới nhất
