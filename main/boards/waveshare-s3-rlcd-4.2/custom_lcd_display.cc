...
void CustomLcdDisplay::RLCD_Init() {
    ...
    RLCD_SendCommand(0xD6);  // Kiểm soát tải NVM
	RLCD_SendData(0x17);
	RLCD_SendData(0x02);

	RLCD_SendCommand(0xD1); // Kích hoạt tăng cường
	RLCD_SendData(0x01);

	RLCD_SendCommand(0xC0); // Kiểm soát điện áp cổng
	RLCD_SendData(0x11);   
	RLCD_SendData(0x04);   

	RLCD_SendCommand(0xC1); // Cài đặt VSHP
	RLCD_SendData(0x69);
	RLCD_SendData(0x69);
	RLCD_SendData(0x69);
	RLCD_SendData(0x69);
...
void CustomLcdDisplay::RLCD_Display() {
    RLCD_SendCommand(0x2A);     // Bộ địa chỉ cột
  	RLCD_SendData(0x12);
  	RLCD_SendData(0x2A);

  	RLCD_SendCommand(0x2B);     // Bộ địa chỉ trang
  	RLCD_SendData(0x00);
  	RLCD_SendData(0xC7);

  	RLCD_SendCommand(0x2c);     // Bộ địa chỉ trang
