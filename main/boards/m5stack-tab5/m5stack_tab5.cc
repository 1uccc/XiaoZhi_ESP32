...
#define LCD_MIPI_DSI_PHY_PWR_LDO_CHAN       3  // LDO_VO3 được kết nối với VDD_MIPI_DPHY
...
public:
    Pi4ioe1(i2c_master_bus_handle_t i2c_bus, uint8_t addr) : I2cDevice(i2c_bus, addr) {
        ...
        WriteReg(PI4IO_REG_IO_DIR, 0b01111111);      // 0: đầu vào 1: đầu ra
        WriteReg(PI4IO_REG_OUT_H_IM, 0b00000000);    // Các chân đã qua sử dụng bị tắt Trở kháng cao
        WriteReg(PI4IO_REG_PULL_SEL, 0b01111111);    // kéo lên/xuống chọn, 0 xuống, 1 lên
        WriteReg(PI4IO_REG_PULL_EN, 0b01111111);     // kéo lên/xuống cho phép, 0 tắt, 1 kích hoạt
        WriteReg(PI4IO_REG_IN_DEF_STA, 0b10000000);  // P1, P7 mặc định ở mức cao
        WriteReg(PI4IO_REG_INT_MASK, 0b01111111);    // Cho phép ngắt P7 0 cho phép, 1 cấm
        WriteReg(PI4IO_REG_OUT_SET, 0b01110110);     // Thanh ghi cổng đầu ra P1(SPK_EN), P2(EXT5V_EN), P4(LCD_RST), P5(TP_RST), P6(CAM)RST đầu ra mức cao
    }
...
public:
    Pi4ioe2(i2c_master_bus_handle_t i2c_bus, uint8_t addr) : I2cDevice(i2c_bus, addr) {
        WriteReg(PI4IO_REG_CHIP_RESET, 0xFF);
        uint8_t data = ReadReg(PI4IO_REG_CHIP_RESET);
        WriteReg(PI4IO_REG_IO_DIR, 0b10111001);      // 0: đầu vào 1: đầu ra
        WriteReg(PI4IO_REG_OUT_H_IM, 0b00000110);    // Các chân đã qua sử dụng bị tắt Trở kháng cao
        WriteReg(PI4IO_REG_PULL_SEL, 0b10111001);    // kéo lên/xuống chọn, 0 xuống, 1 lên
        WriteReg(PI4IO_REG_PULL_EN, 0b11111001);     // kéo lên/xuống cho phép, 0 tắt, 1 kích hoạt
        WriteReg(PI4IO_REG_IN_DEF_STA, 0b01000000);  // P6 mặc định ở mức cao
        WriteReg(PI4IO_REG_INT_MASK, 0b10111111);    // Ngắt P6 cho phép 0 kích hoạt, 1 vô hiệu hóa
        WriteReg(PI4IO_REG_OUT_SET, 0b10001001);     // Thanh ghi cổng đầu ra P0(WLAN_PWR_EN), P3(USB5V_EN), P7(CHG_EN) đầu ra mức cao
    }
...
private:
    ...
    void InitializeGt911TouchPad() {
        ...
        tp_io_config.dev_addr = ESP_LCD_TOUCH_IO_I2C_GT911_ADDRESS_BACKUP; // Thay đổi địa chỉ GT911
    ...
    void InitializeSt7123Display() {
        ...
        // Khai báo tất cả các cấu trúc cấu hình ở trên cùng để tránh sự cố goto
        // Khởi tạo bằng memset để tránh mọi cú pháp khởi tạo có thể gây nhầm lẫn cho trình biên dịch
        esp_lcd_dsi_bus_config_t bus_config;
        esp_lcd_dbi_io_config_t dbi_config;
        esp_lcd_dpi_panel_config_t dpi_config;
        ...
        /* trước tiên hãy tạo bus MIPI DSI, nó cũng sẽ khởi tạo DSI PHY */
        bus_config.bus_id = 0;
        bus_config.num_data_lanes = 2;  // ST7123 sử dụng 2 làn dữ liệu
        bus_config.lane_bit_rate_mbps = 965;  // Tốc độ bit làn đường ST7123
        ...
        // chúng tôi sử dụng giao diện DBI để gửi các lệnh và thông số LCD
        dbi_config.virtual_channel = 0;
        dbi_config.lcd_cmd_bits = 8;  // theo thông số LCD
        dbi_config.lcd_param_bits = 8;  // theo thông số LCD
        ...
        dpi_config.dpi_clock_freq_mhz = 70;  // Tần số xung nhịp ST7123 dpi
        ...
        // Sử dụng các chức năng trình điều khiển ST7123 thực tế
    ...
    void InitializeDisplay() {
        // sau khi thiết lập lại tp, đợi 100ms để đảm bảo bus I2C ổn định
        vTaskDelay(pdMS_TO_TICKS(100));
        // Phát hiện màn hình cảm ứng ST7123 (địa chỉ I2C 0x55)
...
public:
    M5StackTab5Board() : boot_button_(BOOT_BUTTON_GPIO) {
        ...
        I2cDetect();
        InitializePi4ioe();
        InitializeDisplay();  // Tự động phát hiện và khởi tạo màn hình + cảm ứng
        InitializeCamera();
        InitializeButtons();
        SetChargeQcEn(true);
        SetChargeEn(true);
    ...
    // Chức năng điều khiển nguồn BSP
    void SetChargeQcEn(bool en) {
        if (pi4ioe2_) {
            uint8_t value = pi4ioe2_->ReadOutSet();
            if (en) {
                clrbit(value, 5);  // P5 = CHG_QC_EN (bật mức thấp)
            } else {
                setbit(value, 5);
            }
